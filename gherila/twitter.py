...
from .models import TwitterMedia, TwitterTweet, TwitterUser, TwitterUserBiolinks
...
class Twitter(Client):
    def __init__(
        self: "Twitter", auth_token: str, ct0: str, crsf: str, authorization: str, **options
    ):
    ...
    async def get_user(self: "Twitter", username: str):
        ...
        if not data.data:
            raise ParseError("Twitter user data are missing or inaccessible")

        if not data.data.user or not data.data.user.result or not data.data.user.result.core:
            raise ParseError("Twitter user is unavailable or its response format changed")
            ...
            if "urls" in entity:
                for link in entity.get("urls", []):
                    if "expanded_url" in link:
                        biolinks.append(TwitterUserBiolinks(**link))
        ...
        obj = TwitterUser(
            ...
            location=(stats.get("location") or {}).get("location", ""),
            verified=stats.is_blue_verified,
            verified_type=(stats.get("verification") or {}).get("verified_type"),
    ...
    async def get_tweet(self: "Twitter", url: str):
        ...
        """
        url = validate_url(url, {"twitter.com", "www.twitter.com", "x.com", "www.x.com"})
        if not TWITTER_CONTENT.match(url):
            raise ValueError("This is not a valid tweet URL")
        ...
        if not data.data or not data.data.threaded_conversation_with_injections_v2:
            raise ParseError("Twitter timeline is missing or inaccessible")
        result = next(
            (
        ...
        if result and result.get("tweet"):
            result = result.tweet
        if not result or not result.get("core") or not result.get("legacy"):
            raise ParseError("Tweet is unavailable or its response format changed")
        ...
        quote_url = (
            legacy.quoted_status_permalink.expanded if "quoted_status_permalink" in legacy else None
        )
        ...
        return TwitterTweet(
            ...
            views=(result.get("views") or {}).get("count") or 0,
    ...
    async def get_user_tweets(self: "Twitter", username: str):
        ...
        data = await self.session.request(
            ...
            params={
                ...
                "fieldToggles": dumps({"withArticlePlainText": False}),
            },
        )

        if (
            not data.data
            or not data.data.user
            or not data.data.user.result
            or not data.data.user.result.timeline
        ):
            raise ParseError("Twitter user timeline is missing or inaccessible")
        entries = next(
            (
                instruction.entries
        ...
        for entry in entries or []:
            if entry.entryId.startswith("tweet-"):
                ...
                if result and result.get("tweet"):
                    result = result.tweet
                if result and (legacy := result.get("legacy")):
                    text = normalize(
                        "NFKC",
                        (
                            result.note_tweet.note_tweet_results.result.text
                    ...
                    tweets.append(
                        TwitterTweet(
                            ...
                            quote_url=quote_url,
                            hashtags=[h.text for h in legacy.entities.hashtags]
                            if "hashtags" in legacy.entities
                            else [],
                            mentions=[m.screen_name for m in legacy.entities.user_mentions]
                            if "user_mentions" in legacy.entities
                            else [],
                            media=self._media(legacy),
                        )
                    )
