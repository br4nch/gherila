...
export function prepareRuntime({ env, cwd, timeout, signal }) {
  ...
  const args = windows
    ? ['-NoLogo', '-NoProfile', '-NonInteractive', '-ExecutionPolicy', 'Bypass', '-File', launcher, '-PrintPython']
    : [launcher, '--print-python'];
  return new Promise((resolve, reject) => {
    ...
    const child = execFile(command, args, {
      cwd, env, detached: !windows, windowsHide: true, maxBuffer: 1024 * 1024, encoding: 'utf8',
    }, (error, stdout, stderr) => {
      clearTimeout(timer);
      ...
      if (error) {
        ...
        reject(new Error('Gherila automatic setup returned no Python executable.'));
      } else {
        resolve(stdout.trim());
      }
    });
